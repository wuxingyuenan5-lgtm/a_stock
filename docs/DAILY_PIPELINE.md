...
- 全 A 快照优先使用东方财富一次性沪深京 A 股接口；失败才回退到新浪分页接口
- 多组指数实时池并发抓取，缺失指数再单独走历史接口
...
生产层使用两个**分离缓存**，不混写历史序列：

1. 东方财富“创新药”概念板块为主源
   - 历史接口直接提供收盘、成交量、成交额、涨跌幅和换手率
   - 换手率直接使用供应商字段
2. 同花顺“创新药”为备用源
   - GitHub Runner 无法访问东方财富概念接口时，仍可补成交额和收益
   - 备用源没有可靠板块总换手率时，该字段明确为空并在 validation 中告警
