...
import pandas as pd
import requests
TARGET="2026-08-06"; TARGET_COMPACT="20260806"; OUT_TAG="20260805"; DATA=Path("data"); DATA.mkdir(exist_ok=True)
...
def index(secid,name):
    url="https://push2his.eastmoney.com/api/qt/stock/kline/get";params={"secid":secid,"fields1":"f1,f2,f3,f4,f5,f6","fields2":"f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61","klt":"101","fqt":"0","beg":TARGET_COMPACT,"end":TARGET_COMPACT,"lmt":"10","ut":"fa5fd1943c7b386f172d6893dbfba10b"};j=requests.get(url,params=params,headers={"User-Agent":UA,"Referer":"https://quote.eastmoney.com/"},timeout=30).json();ls=(j.get("data") or {}).get("klines") or [];x=ls[-1].split(",");return {"日期":x[0],"指标":name,"数据代码":secid,"收盘点位":float(x[2]),"涨跌幅":float(x[8])/100,"成交额（亿元）":float(x[6])/1e8,"数据源":"东方财富历史接口"}
...
summary=pd.DataFrame([{"日期":TARGET,"上涨家数":u,"下跌家数":d,"平盘家数":fl,"涨停家数":lu,"跌停家数":ld,"有效股票数":len(spot),"全部A股成交额（亿元）":ta,"百亿成交股数":len(hot),"百亿成交额（亿元）":ha,"百亿成交集中度":ha/ta if ta else None,"数据源":"AKShare新浪沪深京A股收盘快照+逐股涨跌停价回推"}]);indices=pd.DataFrame([index("1.000016","上证50"),index("47.800007","Choice微盘"),index("1.000985","中证全指")])
summary.to_csv(DATA/f"market_summary_{OUT_TAG}.csv",index=False,encoding="utf-8-sig");hot.to_csv(DATA/f"turnover_100bn_stocks_{OUT_TAG}.csv",index=False,encoding="utf-8-sig");indices.to_csv(DATA/f"index_snapshot_{OUT_TAG}.csv",index=False,encoding="utf-8-sig");spot.to_csv(DATA/f"all_a_snapshot_{OUT_TAG}.csv",index=False,encoding="utf-8-sig");(DATA/f"metadata_{OUT_TAG}.json").write_text(json.dumps({"target_date":TARGET,"effective":len(spot),"hot":len(hot)},ensure_ascii=False,indent=2),encoding="utf-8");print(summary.to_string(index=False));print(indices.to_string(index=False))
